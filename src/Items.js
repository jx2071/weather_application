...
function Items(props) {
  ...
  useEffect(() => {
    axios
      .get(
        "https://aqttbmen16.execute-api.us-east-1.amazonaws.com/dev/inventory_app_items_fetch"
      )
      .then((res) => {
        setState({ rows: res.data, status: "resolved" });
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={[0]}>
                  ...
                  <td>{row[4]}</td>
                  <td>
                    <Button variant="link" onClick={() => handleAdd(row[0])}>
                      <svg
